...
export default function Dashboard() {
  ...
  type Repo = { id: string; owner: string; repo: string; indexed_at: string; };
  ...
  const deleteRepo = async (repo: Repo) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(`Repository ${repo.repo} deleted`);
      } else {
  ...
  const reindexRepo = async (repo: Repo) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        if (data.message === "Repo already indexed and up to date") toast.success("Repository is already up to date");
        else toast.info(`Indexing started for ${repo.repo}…`);
      } else {
    ...
    <div className="space-y-7 animate-fade-up">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">Your connected repositories.</p>
        </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="rounded-2xl border border-border bg-card/30 p-5 flex flex-col gap-4 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {repos.map((repo: any, index: number) => (
            <div
              ...
              className="group relative rounded-2xl border border-border bg-card/50 p-5 hover:border-border/80 hover:bg-card hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 flex flex-col gap-4"
            >
      ...
      >
        <AlertDialogContent className="bg-card border-border rounded-3xl max-w-md">
          <AlertDialogHeader>
            <div className="size-12 rounded-2xl bg-destructive/10 border border-destructive/20 flex items-center justify-center mb-4">
              <AlertCircle className="size-6 text-destructive" />
            </div>
            <AlertDialogTitle className="text-xl">Delete Repository?</AlertDialogTitle>
            <AlertDialogDescription className="text-muted-foreground">
              This will permanently delete{" "}
              <span className="text-foreground font-semibold">"{deletingRepo?.repo}"</span>{" "}
              and all its indexed data. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2 mt-4">
            <AlertDialogCancel
              className="rounded-xl bg-muted border-border hover:bg-accent cursor-pointer"
              disabled={isDeleting}
            >
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              className="rounded-xl bg-destructive hover:bg-destructive/90 text-white border-0 cursor-pointer shadow-lg"
